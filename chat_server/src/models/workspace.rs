...
mod tests {
    ...
    async fn workspace_should_find_by_name() -> Result<()> {
        ...
        let ws = Workspace::find_by_name("test", &pool).await?.unwrap();
        assert_eq!(ws.name, "test");
        Ok(())
    }
    ...
    async fn workspace_should_fetch_all_chat_users() -> Result<()> {
        ...
        let input = CreateUser::new(&ws.name, "Bob", "bob@qq.com", "dsjakda");
        let user1 = User::create(&input, &pool).await?;

        let input = CreateUser::new(&ws.name, "Alice", "alice@qq.com", "dsjakda");
